...
OUT="${OUT:-/root/autodl-tmp/results/HERA-GUARD-APEX-A1M-SHIP-CLASS01-INNER-OOF-P40-CV3-V1}"
...
FULL="/root/autodl-tmp/results/P03-APEX-FULL-V1"
...
COMMON=(
  ...
  --target-coarse ship --target-fine-ids 0 1 --batch-size 96
)

printf "refitting_class01_source_inner_oof_normal\n" > "$OUT/status.txt"
"$PYTHON" scripts/run_apex_boundary_screen.py refit-normal \
  ...
  --calibration-mode inner_oof "${COMMON[@]}" --output "$OUT/train-normal"

ADMITTED="$($PYTHON - "$OUT/train-normal/train_normal_summary.json" <<'PY'
import json, sys
d=json.load(open(sys.argv[1]))["normal_oof"]
r=d["a1m"]
gate=r["incremental_rescue_ledger"]["per_coarse"]["ship"]["precision_gate_pass"]
print(int(r["delta_quality_vs_nms_control"] > 0 and gate))
PY
)"
if [[ "$ADMITTED" != "1" ]]; then
  echo "stopped: robust class01 rescue failed Normal admission" > "$OUT/hard_skipped.txt"
  printf "complete\n" > "$OUT/status.txt"
  exit 0
fi

# Proxy extraction uses the GPU.  Do not contend with the unique full-data
# backbone training that is already running on this one-card server.
printf "waiting_for_full_backbone_before_proxy\n" > "$OUT/status.txt"
while [[ "$(cat "$FULL/status.txt" 2>/dev/null || true)" != "complete" ]]; do
  sleep 20
done
