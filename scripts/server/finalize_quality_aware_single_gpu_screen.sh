...
CANDIDATE_SUBDIR="${CANDIDATE_SUBDIR:-varifocal-fold0}"

cd "${REPO}"
export PYTHONPATH="${REPO}/src${PYTHONPATH:+:${PYTHONPATH}}"
[[ "$(cat "${OUT}/status.txt")" = complete_ready_for_analysis ]] || {
  echo "quality-aware screen has not completed fixed Hard/Sentinel inference" >&2
  exit 2
}
...
CANDIDATE_WEIGHT="${OUT}/fine/${CANDIDATE_SUBDIR}/runs/foundation/weights/last.pt"
