...
R14_STATUS="${R14_STATUS:-/workspace/results/R1-4-AIRCRAFT-STRUCTURED-ATTRIBUTE.status}"
...
CODE_LOCK="${PROJECT_ROOT}/docs/server/R1_AIRCRAFT_VIEW_CONSISTENCY_TASK_01_CODE_SHA256.txt"

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "R1-5 lock already exists: ${LOCK_DIR}" >&2
  exit 1
fi
...
if [[ -e "${RUN_ROOT}" ]]; then
  echo "R1-5 run root exists; refuse overwrite: ${RUN_ROOT}" >&2
  exit 1
fi
if [[ "$(cat "${R14_STATUS}" 2>/dev/null || true)" != "complete" ]]; then
  echo "R1-4 must finish before R1-5" >&2
  exit 1
fi
mkdir -p "${RUN_ROOT}/audit" "${RUN_ROOT}/smoke" \
  "${RUN_ROOT}/train/view_consistency" \
  "${RUN_ROOT}/bundles/view_consistency" "${RUN_ROOT}/evaluation"
...
"${PYTHON_BIN}" -m pytest -q \
  ...
  tests/test_aircraft_class_center_torch.py tests/test_crossfit_thresholds.py
"${PYTHON_BIN}" -m ruff check \
  src/rsdet/analysis/aircraft_refinement.py \
  src/rsdet/analysis/crossfit_thresholds.py \
  src/rsdet/analysis/oof_detection.py \
  scripts/r1_aircraft_refinement.py scripts/analyze_r1_frozen_condition.py \
  tests/test_aircraft_refinement.py tests/test_aircraft_class_center_torch.py \
  tests/test_crossfit_thresholds.py
...
echo "running_smoke_view_consistency" > "${STATUS_PATH}"
"${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
  ...
  --fold 0 --method view_consistency \
  --output-dir "${RUN_ROOT}/smoke/view_consistency" \
  --device cuda --batch-size 8 --num-workers 2 --smoke

for fold in 0 1 2; do
  echo "training_view_consistency_fold_${fold}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
    --training-manifest "${TRAINING_MANIFEST}" --data-root "${DATA_ROOT}" \
    --weights "${CONVNEXT_WEIGHTS}" --p03-checkpoint "${checkpoints[$fold]}" \
    --fold "${fold}" --method view_consistency \
    --output-dir "${RUN_ROOT}/train/view_consistency/fold_${fold}" \
    --device cuda

  echo "inferring_view_consistency_d4_fold_${fold}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" infer \
    ...
    --checkpoint "${RUN_ROOT}/train/view_consistency/fold_${fold}/final_checkpoint.pt" \
    --fold "${fold}" --method view_consistency \
...
(root / "FINAL_GATE_PASS").write_text("R1_5_TASK_PASS\n", encoding="utf-8")
PY
...
echo "R1-5 complete: ${RETURN_PACKAGE}"
