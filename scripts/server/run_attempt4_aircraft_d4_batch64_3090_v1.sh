...
OUT=${OUT:-/root/autodl-tmp/results/HERA-GUARD-ATTEMPT4-AIRCRAFT-D4-BATCH64-3090-V1}
...
batch32 = json.loads(source.read_text())
batch64 = json.loads(source.read_text())
batch32['aircraft_classifier_model']['batch_objects'] = 32
batch64['aircraft_classifier_model']['batch_objects'] = 64
batch32['workpoint_id'] += '_paired_batch32'
batch64['workpoint_id'] += '_paired_batch64'
(output/'batch32_config.json').write_text(json.dumps(batch32, indent=2)+'\n')
(output/'batch64_config.json').write_text(json.dumps(batch64, indent=2)+'\n')
PY
...
for condition in hard sentinel; do
  ...
  "${PY}" scripts/compare_d4_batch_runtime.py \
    --batch32-predictions "${OUT}/${condition}/batch32_predictions.json" \
    --batch64-predictions "${OUT}/${condition}/batch64_predictions.json" \
    --batch32-summary "${OUT}/${condition}/batch32_runtime.json" \
    --batch64-summary "${OUT}/${condition}/batch64_runtime.json" \
