...
ROOT=${ROOT:-/workspace/results/DFINE-M-CV3-VEHICLE-V1}
OUT=${OUT:-${ROOT}/fold_${FOLD}}
BASE_PY=${BASE_PY:-/workspace/venvs/p06-cu121/bin/python}
VENV=${VENV:-/workspace/venvs/dfine-cu121}
PY=${VENV}/bin/python
CONFIG=${PROJECT}/configs/experiments/dfine_m_fold${FOLD}_40ep.yml
...
test -d "${DFINE}"
test -f "${CONFIG}"
test -f "/workspace/results/Y5-ROT90-CV3-OOF/fold_${FOLD}/split_view.json"
if [[ ! -x "${PY}" ]]; then
...
if ! "${PY}" -c 'import numpy; assert numpy.__version__ == "1.26.4"' 2>/dev/null; then
  "${PY}" -m pip install -q --ignore-installed numpy==1.26.4 scipy==1.15.3
fi

"${PY}" "${PROJECT}/scripts/build_dfine_coco_fold.py" \
  --split-view "/workspace/results/Y5-ROT90-CV3-OOF/fold_${FOLD}/split_view.json" \
  --data-root /root/autodl-tmp/data \
  --output-dir "${OUT}/coco" > "${OUT}/logs/coco.log"

WEIGHT=${OUT}/assets/dfine_m_coco.pth
SOURCE_WEIGHT=${SOURCE_WEIGHT:-/workspace/results/DFINE-M-FOLD0-40EP-V1-R2/assets/dfine_m_coco.pth}
if [[ ! -f "${WEIGHT}" && -f "${SOURCE_WEIGHT}" ]]; then
...
"${PY}" scripts/infer_dfine_coco.py \
  --dfine-root "${DFINE}" --config "${CONFIG}" \
  ...
  --summary "${OUT}/inference_summary.json" > "${OUT}/logs/infer.log" 2>&1

"${PY}" scripts/analyze_single_split_official_frontier.py \
  --gt "${OUT}/coco/instances_val.json" \
  --pred "${OUT}/predictions.json" \
  --output "${OUT}/frontier.json" > "${OUT}/logs/frontier.log" 2>&1
