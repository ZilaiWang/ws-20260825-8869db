...
OUT="${OUT:-/root/autodl-tmp/results/HERA-GUARD-APEX-A1M-SHIP-CLASS01-P40-CV3-V1}"
...
printf "waiting_for_inner_oof_audit\n" > "$OUT/status.txt"
ROBUST="/root/autodl-tmp/results/HERA-GUARD-APEX-A1M-SHIP-INNER-OOF-P40-CV3-V1"
while [[ "$(cat "$ROBUST/status.txt" 2>/dev/null || true)" != "complete" ]]; do
  sleep 20
done
...
printf "refitting_class01_normal\n" > "$OUT/status.txt"
"$PYTHON" scripts/run_apex_boundary_screen.py refit-normal \
  ...
  "${COMMON[@]}" --output "$OUT/train-normal"
