...
R12_STATUS="${R12_STATUS:-/workspace/results/R1-2-AIRCRAFT-CLASS-CENTER.status}"
RESULTS_ROOT="${RESULTS_ROOT:-/workspace/results}"
RUN_ID="R1-4-AIRCRAFT-STRUCTURED-ATTRIBUTE"
RUN_ROOT="${RESULTS_ROOT}/${RUN_ID}"
STATUS_PATH="${RESULTS_ROOT}/${RUN_ID}.status"
LOCK_DIR="${RESULTS_ROOT}/.${RUN_ID}.lock"
CONFIG="${PROJECT_ROOT}/configs/experiments/r1_aircraft_structured_attribute_v1.yaml"
CODE_LOCK="${PROJECT_ROOT}/docs/server/R1_AIRCRAFT_STRUCTURED_ATTRIBUTE_TASK_01_CODE_SHA256.txt"

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "R1-4 lock already exists: ${LOCK_DIR}" >&2
  exit 1
fi
...
if [[ -e "${RUN_ROOT}" ]]; then
  echo "R1-4 run root exists; refuse overwrite: ${RUN_ROOT}" >&2
  exit 1
fi
if [[ "$(cat "${R12_STATUS}" 2>/dev/null || true)" != "complete" ]]; then
  echo "R1-2 must finish before R1-4 to avoid evaluation contention" >&2
  exit 1
fi
mkdir -p "${RUN_ROOT}/audit" "${RUN_ROOT}/smoke" \
  "${RUN_ROOT}/train/structured_attribute" \
  "${RUN_ROOT}/bundles/structured_attribute" "${RUN_ROOT}/evaluation"
...
"${PYTHON_BIN}" -m pytest -q \
  ...
  tests/test_aircraft_attributes.py tests/test_aircraft_class_center_torch.py
"${PYTHON_BIN}" -m ruff check \
  src/rsdet/analysis/aircraft_refinement.py \
  src/rsdet/analysis/aircraft_attributes.py \
  scripts/r1_aircraft_refinement.py tests/test_aircraft_refinement.py \
  tests/test_aircraft_attributes.py tests/test_aircraft_class_center_torch.py
...
for path in \
  ...
  "${P03_FOLD0_CHECKPOINT}" "${P03_FOLD1_CHECKPOINT}" \
  "${P03_FOLD2_CHECKPOINT}" "${R11_ROOT}/FINAL_GATE_PASS" \
  "${R11_ROOT}/bundles/p03" "${R11_ROOT}/bundles/ce" \
  "${R11_ROOT}/bundles/selective_anchor_kd"; do
  [[ -e "${path}" ]] || { echo "missing required asset: ${path}" >&2; exit 1; }
done

echo "running_data_and_attribute_audit" > "${STATUS_PATH}"
...
echo "running_smoke_structured_attribute" > "${STATUS_PATH}"
"${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
  ...
  --fold 0 --method structured_attribute \
  --output-dir "${RUN_ROOT}/smoke/structured_attribute" \
  --device cuda --batch-size 16 --num-workers 2 --smoke

for fold in 0 1 2; do
  echo "training_structured_attribute_fold_${fold}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
    --training-manifest "${TRAINING_MANIFEST}" --data-root "${DATA_ROOT}" \
    --weights "${CONVNEXT_WEIGHTS}" --p03-checkpoint "${checkpoints[$fold]}" \
    --fold "${fold}" --method structured_attribute \
    --output-dir "${RUN_ROOT}/train/structured_attribute/fold_${fold}" \
    --device cuda

  echo "inferring_structured_attribute_d4_fold_${fold}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" infer \
    ...
    --checkpoint "${RUN_ROOT}/train/structured_attribute/fold_${fold}/final_checkpoint.pt" \
    --fold "${fold}" --method structured_attribute \
    --output-dir "${RUN_ROOT}/bundles/structured_attribute" --device cuda
done
...
"${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" evaluate \
  ...
  --attribute-bundle-dir "${RUN_ROOT}/bundles/structured_attribute" \
...
attributes = audit["attribute_taxonomy"]["audit"]
assert attributes["status"] == "pass"
assert attributes["aircraft_class_count"] == 20
assert attributes["dimension_count"] == 5
assert attributes["separated_pair_fraction"] > 0.90
assert decision["status"] == "complete"
assert decision["reference_condition"] == "ce_identity"
assert decision["primary_condition"] == "structured_attribute_identity"
assert decision["maximum_bypass_delta"] <= 1e-12
for fold in range(3):
    runtime = json.loads(
        (root / "bundles" / "structured_attribute" / f"fold_{fold}_runtime.json").read_text()
    )
    assert runtime["status"] == "complete"
(root / "FINAL_GATE_PASS").write_text("R1_4_TASK_PASS\n", encoding="utf-8")
PY
...
echo "R1-4 complete: ${RETURN_PACKAGE}"
