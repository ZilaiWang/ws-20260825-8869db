...
TRAINING_MANIFEST="${TRAINING_MANIFEST:?set TRAINING_MANIFEST to N2-v2 proposal manifest}"
INFERENCE_MANIFEST="${INFERENCE_MANIFEST:?set INFERENCE_MANIFEST to R1-0 inference manifest}"
BASE_LOGITS_DIR="${BASE_LOGITS_DIR:?set BASE_LOGITS_DIR to R1-0 logits}"
...
RUN_ID="R1-1-AIRCRAFT-PROPOSAL-REFINEMENT"
RUN_ROOT="${RESULTS_ROOT}/${RUN_ID}"
STATUS_PATH="${RESULTS_ROOT}/${RUN_ID}.status"
LOCK_DIR="${RESULTS_ROOT}/.${RUN_ID}.lock"
CONFIG="${PROJECT_ROOT}/configs/experiments/r1_aircraft_refinement_v1.yaml"
CODE_LOCK="${PROJECT_ROOT}/docs/server/R1_AIRCRAFT_REFINEMENT_TASK_01_CODE_SHA256.txt"

if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
  echo "R1-1 lock already exists: ${LOCK_DIR}" >&2
  exit 1
fi
...
if [[ -e "${RUN_ROOT}" ]]; then
  echo "R1-1 run root exists; refuse overwrite: ${RUN_ROOT}" >&2
  exit 1
fi
mkdir -p \
  "${RUN_ROOT}/audit" "${RUN_ROOT}/smoke" "${RUN_ROOT}/train" \
  "${RUN_ROOT}/bundles/p03" "${RUN_ROOT}/bundles/ce" \
  "${RUN_ROOT}/bundles/selective_anchor_kd" "${RUN_ROOT}/evaluation"

cd "${PROJECT_ROOT}"
export PYTHONPATH="${PROJECT_ROOT}/src:${PROJECT_ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
echo "running_preflight" > "${STATUS_PATH}"
sha256sum --check "${CODE_LOCK}"
"${PYTHON_BIN}" -m pytest -q tests/test_proposal_reranking.py tests/test_aircraft_refinement.py
...
for method in ce selective_anchor_kd; do
  echo "running_smoke_${method}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
    ...
    --fold 0 --method "${method}" --output-dir "${RUN_ROOT}/smoke/${method}" \
    --device cuda --batch-size 16 --num-workers 2 --smoke
done

for method in ce selective_anchor_kd; do
  for fold in 0 1 2; do
    echo "training_${method}_fold_${fold}" > "${STATUS_PATH}"
    "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" train \
      --training-manifest "${TRAINING_MANIFEST}" --data-root "${DATA_ROOT}" \
      --weights "${CONVNEXT_WEIGHTS}" --p03-checkpoint "${checkpoints[$fold]}" \
      --fold "${fold}" --method "${method}" \
      --output-dir "${RUN_ROOT}/train/${method}/fold_${fold}" --device cuda
  done
done

for fold in 0 1 2; do
  echo "inferring_p03_d4_fold_${fold}" > "${STATUS_PATH}"
  "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" infer \
    ...
    --weights "${CONVNEXT_WEIGHTS}" --checkpoint "${checkpoints[$fold]}" \
    --fold "${fold}" --method p03 --output-dir "${RUN_ROOT}/bundles/p03" --device cuda
  for method in ce selective_anchor_kd; do
    echo "inferring_${method}_d4_fold_${fold}" > "${STATUS_PATH}"
    "${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" infer \
      --inference-manifest "${INFERENCE_MANIFEST}" --data-root "${DATA_ROOT}" \
      --weights "${CONVNEXT_WEIGHTS}" \
      --checkpoint "${RUN_ROOT}/train/${method}/fold_${fold}/final_checkpoint.pt" \
      --fold "${fold}" --method "${method}" \
      --output-dir "${RUN_ROOT}/bundles/${method}" --device cuda
  done
done
...
"${PYTHON_BIN}" scripts/r1_aircraft_refinement.py --config "${CONFIG}" evaluate \
  ...
  --p03-bundle-dir "${RUN_ROOT}/bundles/p03" \
  --ce-bundle-dir "${RUN_ROOT}/bundles/ce" \
  --kd-bundle-dir "${RUN_ROOT}/bundles/selective_anchor_kd" \
...
for method in ("p03", "ce", "selective_anchor_kd"):
    for fold in range(3):
        runtime = json.loads((root / "bundles" / method / f"fold_{fold}_runtime.json").read_text())
        assert runtime["status"] == "complete"
(root / "FINAL_GATE_PASS").write_text("R1_1_TASK_PASS\n", encoding="utf-8")
PY
...
echo "R1-1 complete: ${RETURN_PACKAGE}"
