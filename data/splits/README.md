...
这里只提交冻结 JSON manifest；`paired_trend_v1` 额外保存总计约6.6MB的评估GT快照，
保证比较器不读到另一版本标签。不放图像、权重或个人路径。

- `paired_trend_v1/contract.json`：新方法默认单模型训练/开发/确认固定合同，
  3,136/673/672图；使用说明见[固定测试入口](../../docs/PAIRED_TREND_QUICKSTART.md)。
  旧CV3以下文件只用于历史复现，不再要求每个新方法重新训练三折。

- `dev_v1.json`：B 最初生成的 relaxed 开发划分，保留用于历史复现；
- `dev_v2_airport_proxy_k60.json`：舰船和车辆完全复用 `dev_v1`，MAR20 飞机改用 K=60 机场代理视觉域作为不可拆分 `group_id`。这是历史来源隔离开发划分。
