...
当前机器可读版本是 [`configs/project.yaml`](../configs/project.yaml) 中的
`contract_version: contract_v1`；接口变更时以该字段为准。
...
仓库已有能力及完成状态以 [`README.md`](../README.md) 的“优先复用的公共能力”为入口。
新增预测校验、坐标转换、官方评估或阈值选择代码前先检查对应公共模块；确需不同实现时
保留同一输入输出契约，并在实验记录中说明差异，不要静默维护第二套公共规则。
...
- `score`：有限数，范围 0-1；
- 阈值选择前使用足够低的候选门槛并交付原始分数，不要提前应用正式决策阈值；
