...
2. **阈值统一由 A 选**，成员只交低阈值原始预测，不在同一验证集上拟合阈值后
   报成绩。
3. **时效数据**必须来自 E 的正式 10K 流水线（`runtime_10k` 契约），不拿
   model forward 时间冒充端到端时延。
4. 内部目标（如官方口径 FDR≤0.17）一律按官方 macro 口径计，不用 pooled 替代。
5. 协议版本见 `configs/project.yaml`（`contract_v1` / `official_eval_v1`）；
   规则变化需升级版本，历史结果不改写、不跨版本比较。

## 4. 相关文件

- 官方评分方案 PDF：`../../../../比赛评分方案-V1.5.pdf`（V1.6 更新要点见
  `docs/hub/00_project_status/README.md` 与 `docs/EXPERIMENT_PROTOCOL.md`）
