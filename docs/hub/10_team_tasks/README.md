...
| 成员 | 当前主责 | 现状 | 下一明确交付 |
|---|---|---|---|
| A | 协议、验收、创新实验和最终决策 | **N0 四件套已完成**（cross-fit baseline Recall 0.9176/FDR 0.1990；oracle 定位召回 0.9705；对象证据 manifest + FP_BG 抽检包已产出，见 `N0_EVIDENCE_COMPLETE_20260809.md`） | 组织 P04-F→P03-F 正式 CV3 复验；统一实现细分类/背景拒识对象学生；收集 B 的 FP_BG 审计结果 |
| B | 数据划分、CV3、数据审计 | 两份划分已完成；正式 CV3 v2 已冻结并通过 25 类覆盖及泄漏审计 | **盲审 N0-4 FP_BG 抽检包（`outputs/N0-FP-BG-AUDIT/audit_samples.csv`，324 条，一致性率≥0.85 通过）**；协助核对 source-group bootstrap、TU-160 压力折，不再改动 fold 归属 |
