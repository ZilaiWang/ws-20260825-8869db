...
| 成员 | 当前主责 | 现状 | 下一明确交付 |
|---|---|---|---|
| A | 协议、验收、创新实验和最终决策 | 官方 V1.6 评估、N0 v2、N2 v2 合同和 P2 结论等级已收尾；N1 正式 P03/P04 已完成 | 以收尾报告为起点验收 A—E 创新；发布 M1 fixed-last 大文件；只在重启背景拒识时发起 N0-4 v2 盲审 |
| B | 数据划分、CV3、数据审计 | 两份划分已完成；正式 CV3 v2 已冻结并通过 25 类覆盖及泄漏审计 | 创新数据变体必须保持同一 source group 和 fold；N0-4 v2 只在 A 明确重启 N2 时盲审 |
