...
> 2026-07-23 状态注记：本文主体保留探索阶段的预注册与结果边界。
> `cv3_airport_proxy_k60_v2` 已冻结；现在只需先生成
> `formal_crop_manifest_v2`，再按原冻结工作点复跑
> `tight-224 + natural + seed 42` 三折。当前状态见
...
本实验不包含背景候选、漏检、重复框、大图切片和全局融合，因而不是官方
Recall/FDR，不得作为端到端成绩。现有结果使用 P0-2 探索性同源隔离
划分；正式 CV3 v2 已经完成，冻结工作点尚待按新 fold 重跑。
