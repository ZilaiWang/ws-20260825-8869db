...
这是**主线 3 的核心场景**（10K 大图 → tile 切分 → 同一目标跨 tile 多个框/多个细类 → 聚合归并）。当前只验证了"整图级多框"（去重/投票），**没验证"跨 tile 切片"**。10K 测试图未获得（`docs/E_10K_PIPELINE_REPORT.md` 第 85 行）。

所以：**E 的工作是"完成度约 70%"**——聚合算法本身可靠、性能达标，但核心场景（跨 tile）和真实 10K 测试未完成，这与服务器环境（WP4）和官方 10K 图未获得有关，是客观限制。

## 6. E 说的"服务器和图片数据在哪"—— 澄清

E 问的正是他在做的 10K 大图合并的依赖。当前情况：

| E 需要 | 现状 | 处理 |
|---|---|---|
| 10K 官方测试大图 | **未获得**（官方未公布或未下发） | 用合成图先验证（E 已做）；A 同步官方动态 |
| tile 推理环境 | 服务器 RTX 3090 可用 | 已就绪（`connect.nmb2`） |
| M1 checkpoint | 服务器 `/root/autodl-tmp/M1/fold{0,1,2}_best.pt` | 已就绪 |
| OOF 聚合数据 | 本机 `outputs/M1-CV3-OOF-.../M1-CV3-OOF-aggregate/` | 已就绪 |
| 跨 tile 场景验证 | 需真实 10K 图 | **待官方图下发**；可先用合成大图（E 的 `tiling/synthetic.py`）生成 10K 级合成图跑通 pipeline |
