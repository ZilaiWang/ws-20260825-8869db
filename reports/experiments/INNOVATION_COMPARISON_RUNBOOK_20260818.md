...
```bash
ssh -p <PORT> -o ControlPath=/tmp/n2cfg_ssh <GPU_USER>@<GPU_SERVER_HOST> \
...
for k in Y3-HIER Y5-ROT90 Y4-AFSS; do
  scp -P <PORT> -o ControlPath=/tmp/n2cfg_ssh \
    "<GPU_USER>@<GPU_SERVER_HOST>:/tmp/$k-eval.tar.gz" /tmp/
...
```bash
cd <LOCAL_HOME>/Documents/揭榜挂帅-小样本遥感卫星图像/xh-202625
