...
官方 v4.0 结果为 Ship `0.823344/0.060705`、Aircraft `0.950685/0.032303`、Vehicle `0.831579/0.210000`、时延 `3.978833s`。Shared OTM 与 Aircraft-D4 均获官方同向确认，Vehicle 与 v2.0 完全不变。第五次改为 v4.0 加 Vehicle 24 融合后阈值 `0.550` 的单因素探索；Hard/Sentinel 分别 `+0.3053/+0.3334`，Normal pooled diagnostic `-0.0162`，因此只用于最高分保留规则下的最后机会，不宣称正式准入。
