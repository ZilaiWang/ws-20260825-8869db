...
def slice_coco(
    ...
    *,
    ...
    for image_row in sorted(payload["images"], key=lambda row: int(row["id"])):
        ...
        for annotation in annotations_by_image[int(image_row["id"])]:
            ...
            if not candidates:
                dropped_visibility += 1
                continue
            visibility, tile_index, clipped = max(
        ...
        negative_indices.sort(
            key=lambda index: _stable_empty_key(image_row["file_name"], tiles[index])
        )
        ...
        with Image.open(source_path) as source_image:
            for tile_index in kept_indices:
                ...
                output_path = output_image_root / tile_name
                source_image.crop(tile).save(output_path, format="PNG")
                output_image_id = len(output_images) + 1
                rows = assigned.get(tile_index, [])
